...
from parsers.helpers.xml_helpers import XMLRegion
...
def calculate_separator_regions(xml_page, rule):
    ...
    filtered_table_column_separators = list(
        map(lambda a: a.x, filtered_table_column_separators))
    ...
    for i in range(len(filtered_table_column_separators) - 1):
        region = XMLRegion()
        region.x1 = filtered_table_column_separators[i] / \
            Decimal(100.00) * xml_page.region.x2
        region.x2 = filtered_table_column_separators[i +
                                                     1] / Decimal(100.00) * xml_page.region.x2
