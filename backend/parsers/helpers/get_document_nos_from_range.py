...
def get_document_nos_from_range(range_str, first='1', last='1'):
    ...
    re_exp = re.compile(
        r'^(?:%FIRST(?:[-+]\d+)*%|%LAST(?:[-+]\d+)*%|\d+)(?:-(?:%FIRST(?:[-+]\d+)*%|%LAST(?:[-+]\d+)*%|\d+))?(?:,*(?:%FIRST(?:[-+]\d+)*%|%LAST(?:[-+]\d+)*%|\d+)(?:-(?:%FIRST(?:[-+]\d+)*%|%LAST(?:[-+]\d+)*%|\d+))?)*$')
    ...
    while True:
        parts_matching_regex = re.search(
            r"(%FIRST(?:[-+]\d+)*%|%LAST(?:[-+]\d+)*%)", evaled)

        if parts_matching_regex == None or len(parts_matching_regex.groups()) <= 0:
            break

        part_to_be_resolved = parts_matching_regex.groups()[0]
        resolved_part = part_to_be_resolved

        if "FIRST" in part_to_be_resolved:
            resolved_part = re.sub("FIRST", str(first), resolved_part)
        if "LAST" in part_to_be_resolved:
            resolved_part = re.sub("LAST", str(last), resolved_part)

        calculated_part = re.sub(
            r'%([0-9]+[+-][0-9]+)%', lambda x: str(eval(x.group(1))), resolved_part)
