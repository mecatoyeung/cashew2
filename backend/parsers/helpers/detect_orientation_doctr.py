...
def detect_orientation_doctr(document, page_num, pre_processing, last_preprocessing=None, debug=False):
    ...
    abs_pre_processed_rotated_image_path = pre_processed_rotated_image_path(document, pre_processing, page_num)
    ...
    global doctr_model
    if doctr_model == None:
        doctr_model = ocr_predictor(det_arch='db_resnet50', reco_arch='crnn_vgg16_bn', pretrained=True, detect_orientation=True)

    scores = []
    im = cv2.imread(abs_new_pre_processed_image_path)
    im_gray = cv2.cvtColor(im, cv2.COLOR_BGR2GRAY)
    (threshi, img_bw) = cv2.threshold(im_gray, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
    (h, w ,c) = im.shape
    ...
    row_means_of_cropped_image_rotated_90 = np.mean(cropped_image_rotated_90, axis=1)

    sd_cropped_image_rotated_90 = statistics.pstdev(row_means_of_cropped_image_rotated_90) 
