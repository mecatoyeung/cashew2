...
from parsers.models.stream_class import StreamtClass
...
class Stream(models.Model):
    ...
    rule = models.ForeignKey(
        "Rule", related_name='streams', on_delete=models.CASCADE)
    step = models.IntegerField()
    type = models.CharField(max_length=256, choices=StreamType.choices())
    stream_class = models.CharField(
        max_length=256, choices=StreamtClass.choices())
    ...
    combine_first_n_lines = models.IntegerField(
        null=True, blank=True, default=2)
    convert_to_table_by_specify_headers = models.TextField(
        null=True, blank=True, max_length=4096, default="")
    open_ai_question = models.TextField(
        null=True, blank=True, max_length=4096, default="")
    col_index = models.IntegerField(null=True, blank=True, default=1)
    col_indexes = models.TextField(
        null=True, blank=True, max_length=1024, default="")
    remove_matched_row_also = models.BooleanField(
        null=True, blank=True, default=False)
    unpivot_column_index = models.TextField(
        null=True, blank=True, max_length=1024, default="")
    unpivot_newline_char = models.TextField(
        null=True, blank=True, max_length=1024, default="")
