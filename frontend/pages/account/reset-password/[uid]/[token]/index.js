...
export default function ResetPassword() {
  ...
  const newPassword1ChangeHandler = (e) => {
    setResetPasswordForm(produce((draft) => {
  ...
  const resetPasswordBtnClickHandler = (e) => {
    ...
    service.post("rest-auth/password/reset/confirm/", {
      ...resetPasswordForm,
      uid: uid,
      token: token,
    }, resposne => {
  ...
  return (
    <>
        <h1 className={accountStyles.h1} style={{ display: "block", textAlign: "center" }}>Change Password</h1>
          ...
          <Form>
            ...
            <Row>
              <Form.Group as={Col} className={accountStyles.actions + " " + accountStyles.col + " xs-12" + " md-3"}>
                <Button className={accountStyles.changePasswordBtn + " " + accountStyles.btn}
                        style={{ marginRight: 10 }}
                        onClick={(e) => resetPasswordBtnClickHandler(e)}
                        disabled={resetPasswordForm.submitting}>
                          {resetPasswordForm.submitting && <img className="spinner" src={Loader.src} />}{resetPasswordForm.submitting ? "Loading": "Reset Password"}</Button>
                <Button className={accountStyles.changePasswordBtn + " " + accountStyles.btn}
                  variant="secondary"
                  onClick={(e) => backToSignInBtnClickHandler(e)}
                  disabled={resetPasswordForm.submitting}>
                    {resetPasswordForm.submitting && <img className="spinner" src={Loader.src} />}{resetPasswordForm.submitting ? "Loading": "Back to Sign In page"}</Button>
              </Form.Group>
            </Row>
          </Form>
        </div>
