...
const IntegrationEdtior = ({ 
  ...
  const insertText = (textToInsert) => {
    ...
    let updatedValue = textBeforeCursorPosition + textToInsert + textAfterCursorPosition
    onChange({
        target: {
            value: updatedValue
        }
  ...
  return (
    <Form.Group className="col-12" controlId="integrationEditor">
        ...
        <Dropdown style={{ display: "flex", flexDirection: "row"}}>
            ...
            {rules.map(rule => (
                <Dropdown.Item key={rule.id} style={{ fontSize: "80%" }} onClick={(e) => addParsedResultClickHandler(rule)}>{rule.name}</Dropdown.Item>
            ))}
            </Dropdown.Menu>
        </Dropdown>
        <Dropdown>
